...
try {
    switch ($action) {
        ...
        default:       json_err('Neznámá akce: ' . $action, 404);
    }
} catch (Throwable $e) {
    json_err('Serverová chyba: ' . $e->getMessage(), 500);
}
...
function require_post(): void {
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') json_err('Vyžadováno POST', 405);
}
...
function create_job(): void {
    $name = trim($_POST['name'] ?? 'Bez názvu');
    if (!isset($_FILES['file'])) json_err('Chybí soubor');
    $f = $_FILES['file'];
    if ($f['error'] !== UPLOAD_ERR_OK) json_err('Chyba uploadu (kód ' . $f['error'] . ')');
    if ($f['size'] > MAX_UPLOAD_BYTES) json_err('Soubor je příliš velký');
    ...
    if (in_array($ext, ALLOWED_VIDEO_EXT, true)) {
        ...
        json_err('Nepodporovaný typ souboru: .' . $ext);
    }

    $engine = ($_POST['engine'] ?? 'sam2') === 'rmbg' ? 'rmbg' : 'sam2';
    if ($engine === 'rmbg' && $sourceType !== 'video') {
        json_err('RMBG Luma režim podporuje zatím jen video soubory. Pro sekvence použij SAM2 workflow.');
    }
    ...
    if (!move_uploaded_file($f['tmp_name'], $dest)) json_err('Nelze uložit upload');

    $initialStatus = $engine === 'rmbg' ? 'queued' : 'created';
    $initialMsg    = $engine === 'rmbg' ? 'Ve frontě na RMBG Luma export' : 'Nahráno, čeká na extrakci framů';
    ...
    $st = db()->prepare(
        'INSERT INTO jobs(token,name,source_type,source_path,status,stage_msg,engine,sam_model,matte_enabled,refine_enabled,refine_mode,
                          rmbg_model_id,rmbg_invert,rmbg_blur_radius,rmbg_gamma,rmbg_force_cpu,rmbg_crf)
         VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)'
    );
    // Nové joby jsou výchozí FAST: Base+, MatAnyone OFF, Refine OFF, refine_mode=fast.
    $st->execute([$token, $name, $sourceType, basename($dest), $initialStatus, $initialMsg, $engine, 'hiera_base_plus', 0, 0, 'fast',
                  $rmbgModel, $rmbgInvert, $rmbgBlur, $rmbgGamma, $rmbgCpu, $rmbgCrf]);
    $jobId = (int) db()->lastInsertId();
    job_log($jobId, "Job vytvořen ($engine/$sourceType): " . $f['name']);

    // Pozn.: extrakci framů dělá worker při claimu (ffmpeg/EXR potřebuje GPU box).
    // Frontend zatím nabídne tlačítko „Připravit framy" → enqueue ve stavu 'created'.
...
function save_masks(): void {
    ...
    try {
        ...
        $pdo->rollBack();
        json_err('Uložení masek selhalo: ' . $e->getMessage(), 500);
    }
    job_log($job['id'], 'Masky uloženy (' . count($masks) . ' objektů)');
...
function save_brush(): void {
    ...
    $job = find_job_by_token($token);
    if (!$job) json_err('Job not found', 404);
    if (!isset($_FILES['brush'])) json_err('Chybí brush soubor');
    $f = $_FILES['brush'];
    if ($f['error'] !== UPLOAD_ERR_OK) json_err('Chyba uploadu brush');
    ...
    if (!move_uploaded_file($f['tmp_name'], $dir . '/' . $fname)) {
        json_err('Nelze uložit brush masku');
    }
...
function enqueue_job(): void {
    ...
    db()->prepare(
        'UPDATE jobs SET status=?, stage_msg=?, progress=0, worker_id=NULL, error_msg=NULL,
         ...
         WHERE id=?'
    )->execute(['queued', 'Ve frontě na worker', $samModel, $multi, $matte, $outfmt,
    ...
    job_log($job['id'], "Zařazeno do fronty (SAM=$samModel, multi=$multi, matte=$matte, out=$outfmt, feather=$edgeFeather, shrink=$edgeShrink, choke=$edgeChoke, cleanup=$edgeCleanup, refine=$refineEnabled, hair=$refineHair, radius=$refineRadius, smooth=$refineSmooth, contrast=$refineMaskContrast, halo=$refineLumaHalo, edgeContrast=$refineEdgeContrast)");
...
function get_result(): void {
    ...
    $job = find_job_by_token($token);
    if (!$job) json_err('Job not found', 404);
    if ($job['status'] !== 'done') json_err('Job ještě není hotový', 409);

    $resDir = RESULTS . '/' . $job['token'];
    $matches = glob($resDir . '/result*.*') ?: [];
    if (!$matches) json_err('Výsledek nenalezen', 404);
    ...
    if ($filtered !== null) {
        if (!$filtered) json_err('Požadovaný typ výsledku není k dispozici: ' . $want, 404);
        $matches = $filtered;
    }
...
function delete_jobs_bulk(?array $statuses = null): void {
    ...
    if ($statuses === null) {
        ...
        if (!$statuses) json_err('Neplatný filtr mazání');
...
function cancel_job(): void {
    ...
    db()->prepare("UPDATE jobs SET stage_msg=?, updated_at=datetime('now') WHERE id=?")
       ->execute(['Požadavek na stop odeslán…', $job['id']]);
...
function tracking_preview_image(): void {
    ...
    if (!$job) json_err('Job not found', 404);
    $ip = tracking_preview_image_path($job);
    if (!is_file($ip)) json_err('Tracking preview nenalezen', 404);
    header('Content-Type: image/jpeg');
...
function update_install(): void {
    if (!isset($_FILES['update_zip'])) json_err('Chybí update ZIP');
    $f = $_FILES['update_zip'];
    if (($f['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) {
        json_err('Chyba uploadu update ZIPu (kód ' . (int)$f['error'] . ')');
    }
    $ext = strtolower(pathinfo((string)$f['name'], PATHINFO_EXTENSION));
    if ($ext !== 'zip') json_err('Update musí být ZIP');
    if ((int)$f['size'] <= 0) json_err('Update ZIP je prázdný');
    if ((int)$f['size'] > 512 * 1024 * 1024) json_err('Update ZIP je moc velký');
    ...
    if (!move_uploaded_file($f['tmp_name'], $zipPath)) json_err('Nelze uložit update ZIP');

    $py = update_find_python();
    if (!$py) json_err('Nenalezen Python runtime pro update installer');
    $script = APP_ROOT . '/tools/apply_update.py';
    if (!is_file($script)) json_err('Chybí tools/apply_update.py');
    ...
    if (!is_resource($proc)) json_err('Nelze spustit update installer');
    fclose($pipes[0]);
    ...
    if ($code !== 0 || !is_array($result) || empty($result['ok'])) {
        json_err('Update selhal: ' . trim(($stderr ?: '') . "\n" . ($stdout ?: '')), 500);
    }
...
function _collect_local_system_stats_slow(): array {
    ...
    if (!$gotGpu) $notes[] = 'nvidia-smi nedostupné pro PHP/API';
...
PS;
    ...
    if ($d['cpu_percent'] === null || $d['ram_percent'] === null) $notes[] = 'CPU/RAM nedostupné pro PHP/API';
...
function worker_claim(): void {
    ...
    try {
        ...
        )->execute([$workerId, 'Worker převzal job', $row['id']]);
        $pdo->commit();
    } catch (Throwable $e) {
        $pdo->rollBack();
        json_err('Claim selhal: ' . $e->getMessage(), 500);
    }
    ...
    $st->execute([$row['id']]);
    $job = $st->fetch();
    job_log($job['id'], "Worker '$workerId' převzal job (stav={$job['status']})");
...
function worker_result_meta(): void {
    ...
    job_log($id, 'Worker dokončil výpočet, výsledky nahrány');
    json_out(['ok' => true]);
}
...
function worker_upload_frames(): void {
    ...
    if ($zip->open($tmp) !== true) json_err('ZIP nelze otevřít', 500);
    ...
    job_log($id, "Nahráno $count náhledových framů");
...
function worker_upload_result(): void {
    ...
    // 'file' = surový výsledek (mp4/mov), 'zip' = PNG sekvence / legacy ZIP.
    $field = null;
    ...
    if (!$f) json_err('Chybí výsledný soubor');
    ...
    $dest = $dir . '/' . $destName;
    @unlink($dest); // nepřepisuj ostatní typy výsledků; MP4 a PNG ZIP mohou existovat vedle sebe

    if (!move_uploaded_file($f['tmp_name'], $dest)) {
        json_err('Nelze uložit výsledný soubor', 500);
    }
    job_log($id, 'Result nahrán: ' . $destName . ' (' . round(filesize($dest) / 1048576, 1) . ' MB)');
...
function worker_fail(): void {
    $d = body_json();
    $id  = (int)($d['id'] ?? 0);
    $msg = substr((string)($d['error'] ?? 'neznámá chyba'), 0, 500);
    db()->prepare(
        "UPDATE jobs SET status='error', error_msg=?, stage_msg=?, updated_at=datetime('now')
         WHERE id=?"
    )->execute([$msg, 'Chyba: ' . $msg, $id]);
    job_log($id, 'Worker hlásí chybu: ' . $msg, 'error');
    json_out(['ok' => true]);
}
...
function preview_result(): void {
    ...
    $st->execute([$pid]);
    $pv = $st->fetch();
    if (!$pv) json_err('Preview nenalezen', 404);
...
function preview_mask(): void {
    ...
    if (!is_file($path)) json_err('Soubor masky chybí', 404);
...
function worker_preview_claim(): void {
    ...
    try {
        ...
        $pdo->rollBack();
        json_err('Preview claim selhal: ' . $e->getMessage(), 500);
    }
...
function worker_preview_result(): void {
    ...
    if (!move_uploaded_file($_FILES['mask']['tmp_name'], $dest)) {
        json_err('Nelze uložit masku', 500);
    }
...
function worker_preview_fail(): void {
    $d = body_json();
    $id = (int)($d['id'] ?? 0);
    $msg = substr((string)($d['error'] ?? 'chyba'), 0, 500);
